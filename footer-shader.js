// Subtle animated blue plasma-grid shader for the footer background.
(function () {
  ...
  const fsSource = `
    ...
    const float overallSpeed = 0.06;
    const float gridSmoothWidth = 0.015;
    const float scale = 5.0;
    const vec4 lineColor = vec4(0.18, 0.45, 1.0, 1.0);
    ...
    void main() {
      ...
      for (int l = 0; l < linesPerGroup; l++) {
        ...
        float offset = random(offsetPosition + offsetTime * (1.0 + float(l) / float(linesPerGroup))) * mix(minOffsetSpread, maxOffsetSpread, horizontalFade);
